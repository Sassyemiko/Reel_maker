...
```bash
git clone https://github.com/harvestingmoon/Reels_maker.git
cd Reels_maker
```

Build the Docker image:
```bash
docker build -t Reels_maker:latest .
```

### 2️⃣ Run the Docker Container
Run the container interactively:
```bash
docker run -it --rm -p 8000:5000 Reels_maker:latest /bin/bash
```
This command:
...
- Runs the container using the latest image (`Reels_maker:latest`)
